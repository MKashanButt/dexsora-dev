...
@section('content')
    <div class="space-y-6 relative">
        ...
        <!-- Header -->
        <x-gradient-header title="Data Sheets 📊"
            subtitle="Manage your custom data tables and organize information efficiently" buttonText="Create New Sheet"
            buttonAction="openCreateModal()" buttonIcon="plus" :showFloatingElements="true" />

        <!-- Sheets Grid -->
        @if($sheets->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($sheets as $sheet)
                    <x-card class="cursor-pointer" :clickable="true" :onclick="'window.location.href=\'' . route('sheets.show', $sheet) . '\''">
                        <div class="p-8">
                            <div class="flex items-center justify-between mb-6">
                                <div
                                    class="h-16 w-16 bg-gradient-to-br from-indigo-500 via-purple-500 to-indigo-600 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform shadow-lg">
                                    <x-icon name="table" class="h-8 w-8 text-white" />
                                </div>
                                <span
                                    class="text-xs font-medium text-gray-500 bg-gray-100/80 px-3 py-2 rounded-full">{{ $sheet->updated_at->diffForHumans() }}</span>
                            </div>
                            ...
                            <div class="flex space-x-3">
                                ...
                                <x-button variant="outline" size="sm" onclick="event.stopPropagation(); editSheet({{ $sheet->id }})"
                                    class="flex-1">
        ...
        @else
            <x-empty-state title="No sheets yet"
                description="Get started by creating your first data sheet to organize and manage your information efficiently."
                buttonText="Create Your First Sheet" buttonAction="openCreateModal()" icon="table" />
        @endif
    </div>

    <!-- Create Sheet Modal -->
    <div id="createModal"
        class="fixed inset-0 backdrop-blur-xl bg-black/20 overflow-y-auto h-full w-full hidden z-50 flex items-center justify-center animate-fade-in">
        <div
            class="relative mx-auto p-8 w-full max-w-lg shadow-2xl rounded-3xl bg-white/95 backdrop-blur-xl border border-white/20 animate-scale-in">
            <!-- Modal Header -->
            <div class="mb-8 text-center">
                <div
                    class="h-16 w-16 bg-gradient-to-br from-blue-500 via-purple-500 to-indigo-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg">
